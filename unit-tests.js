$(document).ready(function() {
  ...
  function updateTotals(_passed, _failed) {
    ...
    results.html('<h3>' + _passed + ' passed.</h3><h3>' + _failed + ' failed.</h3><h3>' + duration + ' milliseconds.</h3>');
  }
  ...
  nodeunit.runModule('Unit Tests', suite, {
    ...
    moduleDone : function (name, assertions) {
      ...
      if (failures) {
        status.removeClass('alert-success').addClass('alert-error').html([
        ...
        status.html([
          '<h3>',
            "It's all good! Livestamp.js is working as expected.",
          '</h3>'].join(''));
      }
...
var suite = {
  ...
  '$.fn.livestamp functions': function(test) {
    ...
    tester.livestamp('destroy');
    $.livestamp.update();
    test.equal(tester.eq(0).html(), 'original one', "$.fn.livestamp('add') properly restores the content of first element when two are livestamped together");
    test.equal(tester.eq(1).html(), 'original two', "$.fn.livestamp('add') properly restores the content of second element when two are livestamped together");

    $('#test-area').empty();
    test.done();
  },

  'Event handling': function(test) {
    test.expect(5);

    var tester;

    tester = $('<div>should change</div>').appendTo('#test-area');
    tester.livestamp();
    tester.on('change.livestamp', function(e, from, to) {
      test.equal(e.type, 'change', 'A livestamp about to change triggers the "change" event');
      test.equal(from, 'a few seconds ago', 'The "from" argument to the callback is correct');
      test.equal(to, 'a minute ago', 'The "to" argument to the callback is correct');
    });
    Date.warp.jump('1 minute');
    $.livestamp.update();
    tester.off('change.livestamp');
    Date.warp.reset();

    tester = $('<div></div>').appendTo('#test-area');
    tester.on('change.livestamp', function(e) {
      e.preventDefault();
    });
    tester.livestamp();
    tester.off('change.livestamp');
    test.equal(tester.html(), '', 'Using "preventDefault()" in the callback stops the livestamp from changing');
    ...
    tester.livestamp();
    test.equal(tester.html(), '', 'Using "return false" in the callback stops the livestamp from changing');
